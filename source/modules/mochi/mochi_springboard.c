...
// launch_app()'s install-prompt path (a downloadable remote app,
// app_manager_remote_download()) needs app_manager_remote's download RPCs
// and proximity's capability query — both conditionally-empty on esp32p4/
// tab5 (no radio there at all). Same guard shape cheetah_home.c's own
// CHEETAH_HAS_APP_DOWNLOAD already establishes for the identical reason —
// this is that exact feature, ported: it existed only in cheetah_home.c,
// which is unreachable in any build this tree currently produces (Cheetah
// isn't in purrstrap.py's PURR_WIN_UI_BACKENDS, and no device.pcat sets
// ui="cheetah"), while tdeck_plus — the only device that also enables
// Milkbar — has run ui="mochi" since 2026-09-01. Mochi's own launch_app()
// never gained the equivalent check, so every remote-app tap silently ran
// the app ON THE SERVER instead of ever downloading it — the real bug
// behind "apps not downloading from the server". user_mgr.h has no such
// gate (universal, no radio dependency) and is included unconditionally
// above.
#if defined(CONFIG_IDF_TARGET_ESP32P4)
#define MOCHI_HAS_APP_DOWNLOAD 0
#else
#define MOCHI_HAS_APP_DOWNLOAD 1
...
#if MOCHI_HAS_APP_DOWNLOAD
// ── Remote app install prompt ───────────────────────────────────────────
// Ported from cheetah_home.c's own identically-named section — see this
// file's own header comment (top of file) for why that copy is dead code
// on every device this tree actually builds, and this one isn't. Logic is
// unchanged from the original; only the TAG in notifications and the
// caller (launch_app() below, not Cheetah's launch()) differ.
...
// (app_manager.c's own local-registry calls, independent of remote mode).
static void install_download_task(void *arg)
{
    ...
    if (ok) {
        ...
        purr_kernel_notify("App installed", s_install_name, "mochi_sb");
        ...
        purr_kernel_notify("Install failed", s_install_name, "mochi_sb");
    }
...
static void on_install_yes(purr_wid_t w, purr_event_t e, void *user)
{
    ...
    xTaskCreateWithCaps(install_download_task, "mochi_dl", 4096, NULL, 3, NULL, MALLOC_CAP_SPIRAM);
}
...
// "Run on server" — exactly what a plain remote tap already did before
// this whole feature existed for Mochi. Doesn't download or persist
// anything — the same choice is offered again next time this app is
// tapped, since it's still not installed.
...
// offer_remote=false is the one case where the choice isn't really a
// choice: app->placement == APP_PLACE_LOCAL (author-declared) says this
// app can never run on the server, so there's nothing to offer alongside
// Install.
static void open_install_dialog(int registry_idx, const char *name, const uint8_t mac[6], bool offer_remote)
{
    close_install_dialog();   // never orphan a previous prompt
    ...
    // 64-byte fixed text + up to 47 bytes of name + null = up to 112.
    char msg[128];
    if (offer_remote) {
...
static void launch_app(int registry_idx)
{
    ...
    // every tap offers the real choice: Install, or Run on server. This
    // is the actual fix for "apps not downloading" — see this file's own
    // header comment on why the check never existed here at all.
    ...
    if (app_manager_is_remote() && app_manager_remote_mac(remote_mac)) {
        ...
        // Not downloadable at all (pre-linked/compiled-in, or already
        // personal) — no install choice to offer. Falls straight through
        // to the ordinary remote launch below.
    }
#endif
