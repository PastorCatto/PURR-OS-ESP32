// tabby_win.c — catcall_ui_t backend for Tabby (LVGL).
//
// Adapted from cupcake_win.c rather than written fresh, deliberately: that
// file already encodes several hard-won, live-confirmed fixes that any LVGL
// backend needs and that are not obvious from the contract —
//
...
//
// Re-deriving those would mean re-hitting them on hardware. What Tabby changes
// on top:
//
...
static const char *TAG = "tabby_win";
...
// Shared "currently shown" stack backing tabby_win_hide_foreground(). Only one
...
// group removal is Tabby-specific and load-bearing: because this backend adds
...
// Tabby's addition: make a widget reachable from keyboard/trackball.
...
static purr_win_t tb_win_create(const char *title) {
    ...
    lv_obj_set_size(win, tabby_hal_width(), tabby_hal_height());
...
void tabby_win_hide_foreground(void) {
    ...
    // Nothing is covering the shell any more — put keyboard focus back on it
    // so typing resumes filtering instead of going nowhere.
    tabby_shell_go_home();
}
...
static purr_wid_t tb_btn_create(purr_win_t h, const char *label,
    ...
    group_add(btn);   // Tabby: buttons are keyboard/trackball reachable
    if (cb) {
...
static void tb_list_set_items_async_cb(void *user) {
    ...
    if (list && sctx->wid >= 1 && sctx->wid <= MAX_WIDS) {
        ...
        for (int i = 0; i < sctx->count; i++) {
            ...
            group_add(btn);   // Tabby: rows are keyboard/trackball reachable
            cb_ctx_t *ctx = heap_caps_malloc(sizeof(cb_ctx_t), MALLOC_CAP_DEFAULT);
            if (ctx) {
...
static void tb_kb_show(purr_win_t h, purr_wid_t target) {
    ...
    if (tabby_hal_has_physical_keyboard()) return;
    lv_obj_t *w  = get_win(h);
...
static const catcall_ui_t s_tabby_win = {
    .name            = "tabby",
...
void tabby_win_register(void) {
    purr_kernel_register_ui(&s_tabby_win);
}
