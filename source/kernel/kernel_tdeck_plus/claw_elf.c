...
// verified field-by-field against a real compiled object before this file
// was written (guest.o's actual header bytes decoded by hand once, cross-
// checked against readelf's own output, not copied from a reference without
// checking). __attribute__((packed)) because e_ident's 16 bytes keep every
// following field naturally misaligned relative to a struct start that
// isn't itself 4-aligned in the source buffer — this parser only ever reads
// through memcpy into these structs (never casts a raw pointer to one and
...
#define R_XTENSA_SLOT0_OP  20   // self-relocating, see this file's header comment — never returned as a patch
...
static const char *str_at(const uint8_t *data, size_t len, size_t strtab_off, size_t strtab_size, uint32_t name_off)
{
    ...
    // below, all synchronous within claw_elf_load()).
...
// Finds a section by name, filling *out and *out_idx. Returns false if not
// found (or the ELF is malformed enough that even shstrtab can't be read).
...
// Finds a symbol by name within the given symtab/strtab section pair.
...
bool claw_elf_load(const uint8_t *data, size_t len, const char *entry_symbol, claw_module_t *out)
{
    ...
    elf32_ehdr_t eh;
    if (!read_at(data, len, 0, sizeof(eh), &eh)) {
    ...
    if (eh.e_ident[4] != ELFCLASS32 || eh.e_ident[5] != ELFDATA2LSB) {
        ESP_LOGE(TAG, "not ELF32 LSB (class=%u data=%u)", eh.e_ident[4], eh.e_ident[5]);
        return false;
    }
    if (eh.e_type != ET_REL) {
        ESP_LOGE(TAG, "not a relocatable object (e_type=%u) — must be a plain `-c` compile, never linked", eh.e_type);
        return false;
    }
    if (eh.e_machine != EM_XTENSA) {
        ESP_LOGE(TAG, "wrong machine (e_machine=%u, expected EM_XTENSA=%u)", eh.e_machine, EM_XTENSA);
        return false;
    }
    if (eh.e_shentsize < sizeof(elf32_shdr_t) || eh.e_shstrndx >= eh.e_shnum) {
        ESP_LOGE(TAG, "malformed section header table");
        return false;
    }

    elf32_shdr_t text_sh, rodata_sh, data_sh, bss_sh, symtab_sh, strtab_sh, rela_sh;
    ...
    if (!find_section(data, len, &eh, ".text", &text_sh, &text_idx)) {
        ESP_LOGE(TAG, "no .text section");
        return false;
    }
    if (!find_section(data, len, &eh, ".symtab", &symtab_sh, NULL)) {
        ESP_LOGE(TAG, "no .symtab section");
        return false;
    }
    if (!find_section(data, len, &eh, ".strtab", &strtab_sh, NULL)) {
    ...
    // (round 2's guest2.c minus its .rodata string would be an example)
