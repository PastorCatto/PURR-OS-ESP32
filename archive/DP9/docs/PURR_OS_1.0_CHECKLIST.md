...
| **Minimal-build profile** — measured, not yet a feature | Open — a `tdeck_plus` cut to core OS + Lua + Cupcake/Android + WiFi (20 components instead of 36) booted with **128,103 bytes** free internal DRAM against ~40,700 for the full config: **3.1x**, largest block 15,872 → 31,744, firmware 28% smaller. For scale, Speed Demon exists to reclaim 12–15 KB; simply not loading unused modules freed ~87 KB. Internal SRAM is the binding constraint on this board, so this is the largest single lever found this cycle. Deferred to the planned UI tooling work | 1.0 |
| **`[drivers]` are not freely toggleable on a device with a specialized kernel** | Open — design constraint for the above. `[modules]` and `[apps]` can be turned off from `device.pcat`, but `kernel_tdeck_plus/kernel_tdp_boot.c` calls `sx1262_rl_configure()` directly, so clearing `[drivers] radio` fails at link even with LoRa and the mesh stack off. A driver is only as removable as its kernel allows; a UI that offers driver toggles has to either know this or drive the kernel from the same manifest | 1.0 |
